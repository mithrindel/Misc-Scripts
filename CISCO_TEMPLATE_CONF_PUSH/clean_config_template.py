...
def args():
    ...
    parser.add_argument('-t', '--template',
                    help='Specify a template config to apply',
                    required=True)

    arg = vars(parser.parse_args())

    global main_menu_actions, sub_menu_actions, config_menu_actions, hosts_file, \
        network_devices, int_sts, username_list, enable_list, logging_list, snmp_list, \
        attribute_list
    network_devices = {}
    int_sts = {}
...
def main_menu():
    ...
    print ('\t* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *')
    print ('\t*                                                           *')
    print ('\t*   Welcome to Cisco Template Configurator Assistant        *')
    print ('\t*                                                           *')
    print ('\t*                                                           *')
    print ('\t* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *')
    print ('\n\n\tPlease choose an option from the following:\n\n')
    print ('\t\t1. Show IP addresses of devices to configure in list file\n')
    print ('\t\t2. Show current template configuration settings\n')
...
def sh_cmd_outputs():
    ...
    print ('\n\n\t\t3. Inject Template configuration')
